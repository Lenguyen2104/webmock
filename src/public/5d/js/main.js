...
async function ShowListBD(list_orders) {
    ...
    let result = await list_orders.map((list_orders) => {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            //console.log(i+'-'+info);
            if(i == (info-1)){
                resultData[n] = `<div data-v-34b49f94="" id="p-${id}" class="li lineRed"><div data-v-34b49f94="">${n}</div></div>`;
                id++;
            }
...
$('.foot .right').click(function (e) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            socket.emit('data-server-5', { chane, join, list_join, phone: response.phone, money: value, x, game, ctv: response.ctv, conlai: response.money });
            }
